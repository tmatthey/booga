 ...
 *
 * -----------------------------------------------------------------------------
 *  $Id: AdaptiveTracer.C,v 1.5 1997/02/20 09:41:51 matthey Exp $
 * -----------------------------------------------------------------------------
 */
...
implementRTTI(AdaptiveTracer, Renderer);
#define MINAREA		3
#define SHARED_EDGES 1
AdaptiveTracer::AdaptiveTracer() : Renderer() 
{
  SuperSampleMode = 0;
  mySquare = NULL;
  myRectmode = false;
...
bool AdaptiveTracer::postprocessing()
{  
  return Renderer::postprocessing();
}

bool AdaptiveTracer::doExecute()
{
...
bool AdaptiveTracer::nextSquare()
{
  bool retval = false;
...
void AdaptiveTracer::setRectangle (BoundingRect b)
{ 
  if (myRectmode) {
    myRectmode = false;
...
void AdaptiveTracer::traceRay (int x, int y, int, RGBPixel p)
{
...
void AdaptiveTracer::subdivideToDepth(SSquare *sq, int d)
{
	if (sq->getDepth() == d)
          return;
	if (sq->getLeaf())
...
void AdaptiveTracer::subdivideSquare(SSquare *sq)
{
  ...
  ostrstream os;
  os << "[AdaptiveTracer::subdivideSquare]: sq=" << *sq;
...
void AdaptiveTracer::recomputeStats(SSquare *sq)
{
	Real maxp;
	int in[4];
...
SSquare *AdaptiveTracer::selectSquare(SSquare *list)
{
...
void AdaptiveTracer::draw(SSquare *sq)
{
...
bool AdaptiveTracer::recomputePriority(SSquare *sq /*,ClosedValue SSCLOSED*/)
{
  if (!overlapsRect(sq)) {
    sq->setClosedValue(SSquare::SSCLOSED);
    return false;
...
bool AdaptiveTracer::overlapsRect(SSquare *s)
{
  if (!myRectmode || 
...
{
  if (i < 0 || i > 3) {
    Report::warning("[AdaptiveTracer::getColor]: index out of range");
    return NULL;   
  } else
    return &myColor[i];
}
