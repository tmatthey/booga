 ...
 *
 * -----------------------------------------------------------------------------
 *  $Id: tess-construct.C,v 1.1 1996/01/18 18:15:22 streit Exp $
 * -----------------------------------------------------------------------------
 */
...
static int newnode()
{
  if (q_idx < QSIZE)
    ...
    {
      Report::recoverable("newnode: Query-table overflow");
      return -1;
    }
}
...
static int newtrap()
{
  if (tr_idx < TRSIZE)
    ...
    {
      Report::recoverable("newtrap: Trapezoid-table overflow");
      return -1;
    }
}
