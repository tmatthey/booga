...
#include "booga/glosmesa/vbxform.h"
#include "booga/glosmesa/xform.h"
#endif
...
void gl_Color3f( GLcontext *ctx, GLfloat red, GLfloat green, GLfloat blue )
{
  ...
  __asm {
    mov edx,ctx
    mov ecx,[edx]GLcontext.Visual
    lea eax,[edx]GLcontext.Current.ByteColor[0]
    lea ebx,[ecx]GLvisual.RedScale
    fld dword ptr [ebx]
    fld dword ptr [ebx+4]
    fld dword ptr [ebx+8]
    ...
    fistp gtmp
    fistp rtmp
    mov cl,byte ptr atmp
    mov ch,byte ptr btmp
    bswap ecx
    mov cl,byte ptr rtmp
    mov ch,byte ptr gtmp
