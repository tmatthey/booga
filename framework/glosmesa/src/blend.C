...
#ifdef PC_HEADER
#include "booga/glosmesa/all.h"
#else
#include <assert.h>
#include <stdlib.h>
#include "booga/glosmesa/alphabuf.h"
#include "booga/glosmesa/blend.h"
#include "booga/glosmesa/context.h"
#include "booga/glosmesa/dlist.h"
#include "booga/glosmesa/macros.h"
#include "booga/glosmesa/pb.h"
#include "booga/glosmesa/span.h"
                      ...
                      const GLubyte bdest[], const GLubyte adest[] )
{
   ...
   if (ctx->Color.BlendEquation==GL_FUNC_ADD_EXT
	 ...
	 if (ctx->Visual->EightBitColor) {
	    GLint sR, sG, sB, sA;		/* Source scaling */
	    GLint dR, dG, dB, dA;		/* Dest scaling */

	    /* calculate scalefactors if constant over i: */	 
	    switch(ctx->Color.BlendSrc) {
	       case GL_CONSTANT_COLOR:
		  sR = (GLint) ctx->Color.BlendColor[0] * 255.0F;
		  sG = (GLint) ctx->Color.BlendColor[1] * 255.0F;
		  sB = (GLint) ctx->Color.BlendColor[2] * 255.0F;
		  sA = (GLint) ctx->Color.BlendColor[3] * 255.0F;
		  break;
	       case GL_CONSTANT_ALPHA:
		  sR = sG = sB = sA = (GLint) ctx->Color.BlendColor[3] * 255.0F;
		  break;
	       case GL_ONE_MINUS_CONSTANT_COLOR:
		  sR = (GLint) (1.0F - ctx->Color.BlendColor[0]) * 255.0F;
		  sG = (GLint) (1.0F - ctx->Color.BlendColor[1]) * 255.0F;
		  sB = (GLint) (1.0F - ctx->Color.BlendColor[2]) * 255.0F;
		  sA = (GLint) (1.0F - ctx->Color.BlendColor[3]) * 255.0F;
		  break;
	       case GL_ONE_MINUS_CONSTANT_ALPHA:
		  sR = sG = sB = sA = (GLint) (1.0F - ctx->Color.BlendColor[3]) * 255.0F;
		  break;
	       default:
		  ;
	    }
	    ...
	    switch(ctx->Color.BlendDst) {
	       case GL_CONSTANT_COLOR:
		  dR = (GLint) ctx->Color.BlendColor[0] * 255.0F;
		  dG = (GLint) ctx->Color.BlendColor[1] * 255.0F;
		  dB = (GLint) ctx->Color.BlendColor[2] * 255.0F;
		  dA = (GLint) ctx->Color.BlendColor[3] * 255.0F;
		  break;
	       case GL_CONSTANT_ALPHA:
		  dR = dG = dB = dA = (GLint) ctx->Color.BlendColor[3] * 255.0F;
		  break;
	       case GL_ONE_MINUS_CONSTANT_COLOR:
		  dR = (GLint) (1.0F - ctx->Color.BlendColor[0]) * 255.0F;
		  dG = (GLint) (1.0F - ctx->Color.BlendColor[1]) * 255.0F;
		  dB = (GLint) (1.0F - ctx->Color.BlendColor[2]) * 255.0F;
		  dA = (GLint) (1.0F - ctx->Color.BlendColor[3]) * 255.0F;
		  break;
	       case GL_ONE_MINUS_CONSTANT_ALPHA:
		  dR = dG = dB = dA = (GLint) (1.0F - ctx->Color.BlendColor[3]) * 255.0F;
		  break;
	       default:
		  ;
	    }
	    ...
	    for (i=0;i<n;i++) {
		  ...
		  switch(ctx->Color.BlendEquation) {
		     case GL_FUNC_ADD_EXT:
			...
			break;
		     case GL_FUNC_SUBTRACT_EXT:
			red[i]	 = MAX2((Rss - Rds) / 255, 0);
			green[i] = MAX2((Gss - Gds) / 255, 0);
			blue[i]	 = MAX2((Bss - Bds) / 255, 0);
			alpha[i] = MAX2((Ass - Ads) / 255, 0);
			break;
		     case GL_FUNC_REVERSE_SUBTRACT_EXT:
			red[i]	 = MAX2((Rds - Rss) / 255, 0);
			green[i] = MAX2((Gds - Gss) / 255, 0);
			blue[i]	 = MAX2((Bds - Bss) / 255, 0);
			alpha[i] = MAX2((Ads - Ass) / 255, 0);
			break;
		  }
	       }
	    }
