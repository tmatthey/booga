 ...
 *
 * -----------------------------------------------------------------------------
 *  $Id: GLRenderer.C,v 1.52 1998/04/29 12:04:58 buhlmann Exp $
 * -----------------------------------------------------------------------------
 */
...
Traversal::Result GLRenderer::visit(TriangleList3D* list)
{
#ifdef HAVE_OPENGL
 
  if (myRenderingQuality <= WIREFRAME_GOURAUD) {
    glBegin(GL_LINES);
    for (long i=0; i<list->countSubobject(); i++) {

      const Vector3D& v1 = list->getTriangle(i)->getVertex(0);
      const Vector3D& v2 = list->getTriangle(i)->getVertex(1);
      const Vector3D& v3 = list->getTriangle(i)->getVertex(2);
      glVertex3f((GLfloat)v1.x(), (GLfloat)v1.y(), (GLfloat)v1.z());
      glVertex3f((GLfloat)v2.x(), (GLfloat)v2.y(), (GLfloat)v2.z());
      glVertex3f((GLfloat)v3.x(), (GLfloat)v3.y(), (GLfloat)v3.z());
      glVertex3f((GLfloat)v1.x(), (GLfloat)v1.y(), (GLfloat)v1.z());
    ...
    if (o) {
      ...
      list->adoptUserData(new GLDisplayListOption(l));
      
      glBegin(GL_TRIANGLES);
      
      for (long i=0; i<list->countTriangles(); i++) {
        const Vector3D& v1 = list->getTriangle(i)->getVertex(0);
        const Vector3D& v2 = list->getTriangle(i)->getVertex(1);
        const Vector3D& v3 = list->getTriangle(i)->getVertex(2);
        const Vector3D& n1 = list->getTriangle(i)->getNormal(0);
        const Vector3D& n2 = list->getTriangle(i)->getNormal(1);
        const Vector3D& n3 = list->getTriangle(i)->getNormal(2);
  ...
  myTriangleCount += list->countSubobject();
...
Traversal::Result GLRenderer::visit(Polygon3D* polygon)
{
  ...
  if (myRenderingQuality > WIREFRAME_GOURAUD) {
    ...
    gluTessCallback(tobj, GLU_VERTEX, (void (__stdcall *)())glVertex3dv);
    gluTessCallback(tobj, GLU_END, glEnd);
#else
    gluTessCallback(tobj, (GLenum)GLU_BEGIN, (void (*)())glBegin );
    gluTessCallback(tobj, (GLenum)GLU_VERTEX, (void (*)())glVertex3dv);
    ...
    for (long i=0; i<numVertices; i++) {
      ...
      gluTessVertex(tobj, vertex, &v.item(i)[0]);
    }
...
Traversal::Result GLRenderer::visit(Point3D* p)
{
#ifdef HAVE_OPENGL
  glPointSize(p->getSize());
  glBegin(GL_POINTS);
  glVertex3f(p->getPoint().x(), p->getPoint().y(), p->getPoint().z());
  glEnd();
#endif // HAVE_OPENGL 
  return Traversal::CONTINUE;
}
...
Traversal::Result GLRenderer::visit(LevelOfDetail3D* lod)
{
#ifdef HAVE_OPENGL
...
// get center of boundingbox and convert it to world coordinates:
  ...
  // The camera is at (0,0,0), the distance of the object is the length
  // of the transformed vector: 

  Real distance = (c*getGLMatrix()).length();

  long i;    
  for (i=0; i<lod->countDistances(); i++) {
    if (distance < lod->getDistance(i))
      break;
  }

  lod->setActiveObject(i);
...
Traversal::Result GLRenderer::dispatch(Makeable* obj)
{
  ...
  tryConcrete(TriangleList3D,  obj);
  tryConcrete(LevelOfDetail3D, obj);
